...
public:
  ...
  Groups(const vector<string>& groupFileNames, const char* groupItemSeparator, const char* groupDimensionItemsSeparator, const vector<map<const string, const unsigned int> >& labels2Ids, const vector<unsigned int>& dimensionOrder, const vector<unsigned int>& groupMinSizes, const vector<unsigned int>& groupMaxSizes, const char* groupMinRatiosFileName);
  ~Groups();
