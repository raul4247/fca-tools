...
Tree::Tree(const char* dataFileName, const char* dontCareSetFileName, const vector<float>& deltaVectorParam, const vector<unsigned int>& minSizesParam, const vector<unsigned int>& maxSizesParam, const float minAreaParam, const float maxAreaParam, const vector<float>& areaCoefficientsParam, const float minimizationRatioParam, const vector<string>& groupFileNames, const vector<unsigned int>& groupMinSizes, const vector<unsigned int>& groupMaxSizes, const char* groupMinRatiosFileName, const vector<unsigned int>& cliqueDimensionsParam, const char* inputItemSeparator, const char* inputDimensionSeparator, const char* groupItemSeparator, const char* groupDimensionItemsSeparator, const char* outputFileName, const char* outputItemSeparatorParam, const char* outputDimensionSeparatorParam, const char* patternSizeSeparatorParam, const char* sizeSeparatorParam, const char* sizeAreaSeparatorParam, const char* emptySetStringParam, const char* allSetStringParam, Tree::Flags flags): potential(), absent(), groups()
{
  ...
  allSetString = allSetStringParam;
...
void Tree::printNVector(const vector<vector<unsigned int> >& nVector, ostream& out)
{
  ...
  for (unsigned int dimensionId = 0; dimensionId != n; ++dimensionId)
      ...
      else
	{
      ...
      const vector<unsigned int>& dimension = nVector[*dimensionOrderIt++];
      if (isAMinimization && dimension.size() == *nbOfElementsInDiInFilesIt++)
	{
	  out << allSetString;
	}
      else
	{
	  printDimension(dimension, *ids2LabelsIt, out);
	}
      ++ids2LabelsIt;
    }
}
...
void Tree::peel()
{
  const unsigned int dimensionToPeelId = nextDimension();
	  ...
	  if (nbOf0ToReachForDeltaMaximality == 0)
	    {
	      ...
	      // TODO: deltaMaximalOnEveryDimensionBut(dimensionToPeelId)
...
const bool Tree::notALeaf()
{
  const vector<vector<unsigned int>>::iterator potentialEnd = potential.end();
  vector<vector<unsigned int>>::iterator potentialElementsIt = potential.begin();
  for (; potentialElementsIt != potentialEnd && potentialElementsIt->empty(); ++potentialElementsIt)
    {
    }
  if (potentialElementsIt == potentialEnd)
    {
      if (constraintsSatisfied() && deltaMaximalOnEveryDimension())
	{
	  goodLeaf();
	}
      return false;
    }
  const vector<vector<unsigned int>>::iterator nonEmptyPotentialIt = potentialElementsIt;
  for (++potentialElementsIt; potentialElementsIt != potentialEnd && potentialElementsIt->empty(); ++potentialElementsIt)
    {
    }
  if (potentialElementsIt == potentialEnd)
    {
      vector<unsigned int>& presentElements = present[nonEmptyPotentialIt - potential.begin()];
      presentElements.insert(presentElements.end(), nonEmptyPotentialIt->begin(), nonEmptyPotentialIt->end());
      nonEmptyPotentialIt->clear();
      if (constraintsSatisfied() && deltaMaximalOnEveryDimension())
	{
	  goodLeaf();
	}
  ...
  return constraintsSatisfied() && deltaMaximalOnEveryDimension();
}  

const unsigned int Tree::nextDimension() const
{
#if ENUMERATION_PROCESS == 0
  unsigned int nbOfEmptyDimensions = 0;
  vector<vector<unsigned int>>::const_iterator bestEmptyDimensionIt;
  vector<vector<unsigned int>>::const_iterator bestNonEmptyDimensionIt;
  unsigned int minNbOfPotentialElementsInEmptyDimension = numeric_limits<unsigned int>::max();
  unsigned int minNbOfPotentialElementsInNonEmptyDimension = numeric_limits<unsigned int>::max();
  vector<vector<unsigned int>>::const_iterator potentialElementsIt = potential.begin();
  for (const vector<unsigned int>& presentElements : present)
    {
      if (presentElements.empty())
	{
	  if (!potentialElementsIt->empty() && potentialElementsIt->size() < minNbOfPotentialElementsInEmptyDimension && (find(cliqueDimensions.begin(), cliqueDimensions.end(), potentialElementsIt - potential.begin()) == cliqueDimensions.end() || cliqueDimensions.front() == potentialElementsIt - potential.begin()))
	    {
	      minNbOfPotentialElementsInEmptyDimension = potentialElementsIt->size();
	      bestEmptyDimensionIt = potentialElementsIt;
	    }
	  ++nbOfEmptyDimensions;
	}
      else
	{
	  if (!potentialElementsIt->empty() && potentialElementsIt->size() < minNbOfPotentialElementsInNonEmptyDimension && (find(cliqueDimensions.begin(), cliqueDimensions.end(), potentialElementsIt - potential.begin()) == cliqueDimensions.end() || cliqueDimensions.front() == potentialElementsIt - potential.begin()))
	    {
	      minNbOfPotentialElementsInNonEmptyDimension = potentialElementsIt->size();
	      bestNonEmptyDimensionIt = potentialElementsIt;
	    }
	}
      ++potentialElementsIt;
    }
  if (nbOfEmptyDimensions > 1 || minNbOfPotentialElementsInEmptyDimension < minNbOfPotentialElementsInNonEmptyDimension)
    {
#ifdef VERBOSE_DIM_CHOICE
      cout << "Choosing empty dimension " << reverseDimensionOrder[bestEmptyDimensionIt - potential.begin()] << " with " << minNbOfPotentialElementsInEmptyDimension << " potential elements" << endl;
#endif
      return bestEmptyDimensionIt - potential.begin();
    }
#ifdef VERBOSE_DIM_CHOICE
  cout << "Choosing non-empty dimension " << reverseDimensionOrder[bestNonEmptyDimensionIt - potential.begin()] << " with " << minNbOfPotentialElementsInNonEmptyDimension << " potential elements" << endl;
#endif
  return bestNonEmptyDimensionIt - potential.begin();
#else
  vector<vector<unsigned int>>::const_iterator bestDimensionIt;
  int bestAppeal = -1;
  const vector<vector<unsigned int>>::const_iterator potentialEnd = potential.end();
  for (vector<vector<unsigned int>>::const_iterator potentialElementsIt = potential.begin(); potentialElementsIt != potentialEnd; ++potentialElementsIt)
    {
      if (!potentialElementsIt->empty() && (find(cliqueDimensions.begin(), cliqueDimensions.end(), potentialElementsIt - potential.begin()) == cliqueDimensions.end() || cliqueDimensions.front() == potentialElementsIt - potential.begin()))
	{
	  const int appeal = getAppeal(potentialElementsIt - potential.begin());
#ifdef VERBOSE_DIM_CHOICE
	  cout << "Appeal of dimension " << reverseDimensionOrder[potentialElementsIt - potential.begin()] << ": " << appeal << endl;
#endif
	  if (appeal > bestAppeal)
	    {
	      bestAppeal = appeal;
	      bestDimensionIt = potentialElementsIt;
	    }
	}
    }
  return bestDimensionIt - potential.begin();
#endif
}

#if ENUMERATION_PROCESS == 1
const int Tree::getAppeal(const unsigned int dimensionId) const
{
  int testedArea = 0;
  vector<vector<unsigned int> >::const_iterator potentialElementsIt = potential.begin();
  for (unsigned int i = 0; i != n; ++i)
    {
      if (i != dimensionId)
	{
	  int yesFactor = potentialElementsIt->size();
	  vector<vector<unsigned int> >::const_iterator presentElementsIt = present.begin();
	  for (unsigned int j = 0; j != n; ++j)
	    {
	      if (j != dimensionId && j != i)
		{
		  yesFactor *= presentElementsIt->size();
		}
	      ++presentElementsIt;
	    }
...
const bool Tree::deltaMaximalOn(const unsigned int dimensionId)
{
#ifdef DETAILED_TIME
  clock_t start = clock();
#endif
...
const bool Tree::deltaMaximalOnEveryDimension()
{
  const vector<unsigned int>::const_iterator cliqueDimensionsBegin = cliqueDimensions.begin();
  const vector<unsigned int>::const_iterator cliqueDimensionsEnd = cliqueDimensions.end();
  unsigned int dimensionId = 0;
  for (; dimensionId != n && ((find(cliqueDimensionsBegin, cliqueDimensionsEnd, dimensionId) != cliqueDimensionsEnd && dimensionId != *cliqueDimensionsBegin) || deltaMaximalOn(dimensionId)); ++dimensionId)
    {
    }
...
vector<unsigned int> Tree::fetchElementsInClosureOn(const unsigned int dimensionId)
{
#ifdef DETAILED_TIME
  clock_t start = clock();
#endif
  ...
  const vector<unsigned int> newPresentView = presentView;
  presentView.clear();
...
void Tree::fetchElementsInClosureOnEveryDimension(const bool isDimensionToPeelInTheClique)
{
  ...
  for (unsigned int dimensionId = 0; dimensionId != n ; ++dimensionId)
    {
      vector<unsigned int>::const_iterator cliqueDimensionIt = find(cliqueDimensionsBegin, cliqueDimensionsEnd, dimensionId);
      if (cliqueDimensionIt == cliqueDimensionsEnd)
	{
	  const vector<unsigned int> newItemIds = fetchElementsInClosureOn(dimensionId);
	  present[dimensionId].insert(present[dimensionId].end(), newItemIds.begin(), newItemIds.end());
	...
	{
	  if (cliqueDimensionIt == cliqueDimensionsBegin)
	    {
	      const vector<unsigned int> newItemIds = fetchElementsInClosureOn(dimensionId);
	      present[dimensionId].insert(present[dimensionId].end(), newItemIds.begin(), newItemIds.end());
	      removeIrrelevantElementsFromAbsent(dimensionId, newItemIds, isDimensionToPeelInTheClique);
	      groups.setPresent(dimensionId, newItemIds);
	      while (++cliqueDimensionIt != cliqueDimensionsEnd)
		{
		  potential[*cliqueDimensionIt] = potential[dimensionId];
		  present[*cliqueDimensionIt].insert(present[*cliqueDimensionIt].end(), newItemIds.begin(), newItemIds.end());
		  removeIrrelevantElementsFromAbsent(*cliqueDimensionIt, newItemIds, isDimensionToPeelInTheClique);
		  groups.setPresent(*cliqueDimensionIt, newItemIds);
		}
	    }
	}
    }
...
vector<unsigned int> Tree::presentCardinals() const
{
  ...
  for (const vector<unsigned int>& presentElements : present)
    {
      presentCardinalsVector.push_back(presentElements.size());
    }
...
void Tree::keepPresentPrefixes(const vector<unsigned int> indices)
{
  ...
  for (vector<unsigned int>& presentElements : present)
    {
      presentElements.resize(*indexIt++);
    }
}
