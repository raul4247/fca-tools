...
class Tree
{
 public:
  ...
  static const char* allSetString;
  ...
  Tree();
  Tree(const char* dataFileName, const char* dontCareSetFileName, const vector<float>& deltaVector, const vector<unsigned int>& minSizes, const vector<unsigned int>& maxSizes, const float minArea, const float maxArea, const vector<float>& areaCoefficients, const float minimizationRatio, const vector<string>& groupFileNames, const vector<unsigned int>& groupMinSizes, const vector<unsigned int>& groupMaxSizes, const char* groupMinRatiosFileName, const vector<unsigned int>& cliqueDimensions, const char* inputItemSeparator, const char* inputDimensionSeparator, const char* groupItemSeparator, const char* groupDimensionItemsSeparator, const char* outputFileName, const char* outputItemSeparator, const char* outputDimensionSeparator, const char* patternSizeSeparator, const char* sizeSeparator, const char* sizeAreaSeparator, const char* emptySetString, const char* allSetString, Tree::Flags flags);
...
/* #endif */
  ...
  void goodLeaf() const;
  const bool notALeaf();
  const unsigned int nextDimension() const;
#if ENUMERATION_PROCESS == 1
  const int getAppeal(const unsigned int dimensionId) const;
#endif
  const bool deltaMaximalOn(const unsigned int dimensionId);
  const bool deltaMaximalOnEveryDimension();
  ...
  vector<unsigned int> fetchElementsInClosureOn(const unsigned int dimensionId);
