...
int main(int argc, char* argv[])
{
  ...
  try
	...
	("density", value<float>(&densityThreshold), "set threshold between 0 (completely dense storage) and 1 (minimization of memory usage) to trigger a dense storage of the data (by default 0)")
	...
	("all", value<string>(&allSet), string("combined with --minimize, set string specifying the all set in output data (by default \"" + allSet + "\")").c_str())
      ...
      if (vm.count("help"))
	{
      ...
      if (vm.count("version"))
	{
	  cout << "d-peeler version 2.13" << endl;
	  return EX_OK;
	}
      ...
      if (!vm.count("out"))
	{
      ...
      SparseItemset::setDensityLimit(densityThreshold);
    }
